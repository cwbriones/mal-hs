...
eval (List [Symbol "let*", List bindings, expr]) = do
    env <- get
...
apply (Lambda vars expr:args) = do
    ...
    boundEnv <- liftIO $ foldM bind childEnv (zip vars args)
    withinEnv boundEnv (eval expr)
  where
    bind e (var, val) = insert var val e >> return e
...
withinEnv :: MalEnv       -- ^The environment to perform our action within
          ...
          -> MalIO MalVal
withinEnv temp action = do
    env <- get
    put temp
    val <- action `catchError` restoreEnv env
    put env
    return val
  where
...
initEnv = do
    env <- empty
    foldM_ defBuiltin env builtins
    return env
  where
    defBuiltin e (var, f) = insert var (Func (Fn f)) e >> return e
    builtins = [("+", binOp (+))
               ,("-", binOp (-))
               ,("*", binOp (*))
               ,("/", binOp Prelude.div)
